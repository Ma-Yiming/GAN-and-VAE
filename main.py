...
class Config(object):
    ...
    save_path = 'imgs1/'  # 生成图片保存路径

    vis = False  # 是否使用visdom可视化
    env = 'GAN'  # visdom的env
    plot_every = 20  # 每间隔20 batch，visdom画图一次

    debug_file = 'debug/debug.txt'  # 存在该文件则进入debug模式
    ...
    for epoch in iter(epochs):
        for ii, (img, _) in enumerate(dataloader):
            ...
            if ii % opt.d_every == 0:
                ...
                if output.size() == true_labels.size():
                    ...
                    print('error_d: {}'.format(error_d))
                    errord.append(error_d)
            ...
            if ii % opt.g_every == 0:
                ...
                if output.size() == true_labels.size():
                    ...
                    error_g.backward()
                    print('error_g: {}'.format(error_g))
                    errorg.append(error_g)
                    optimizer_g.step()
